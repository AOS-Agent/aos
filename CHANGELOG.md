...
Summary: Agent-Driven Kanban Phase 0 — "make the board honest." Kanban, list, table, and Today are views over ONE work data model; this fixes the DATA so every view inherits the truth. The board had been lying: it read "0 active" while sessions held live tasks, done items lingered, and the inbox — including the new ambient commitment proposals — was invisible. Root cause was a truncated read plus three tables that never existed in the live `work.db`. No new state machine (that is Phase 1); these are near-zero-risk truth repairs.

- Honest counts + honest working set (`api/work.py`, `adapters/work.py`). `/api/work` counted `by_status` over the 200 most-recently-*created* tasks — a window that excluded the (older) active tasks, so the header read "0 active" while work was in flight. It now returns `adapter.summary()` (authoritative whole-table counts) and `board_tasks()` — the honest working set: every open task (todo/active/waiting) plus a bounded tail of recently-closed ones — never a truncated created-order sample. Header chips in Today derive from the summary; the board shows the real active column.
- Audit trail (`entity_history`), status + comment tables. Migration 050 seeded `work.db` from the tasks tables only, so three tables the canonical schema already defines — `statuses`, `entity_history`, `comments` — never existed there, and `/api/tasks/{id}/activity`, `/api/statuses`, `/api/tasks/{id}/comments` returned `[]` regardless of data. Those endpoints ALSO referenced `ontology._work_adapter` (never set) and `db.conn` (the adapter exposes `_conn`), so they were dead even once tables existed. The WorkAdapter now ensures the aux schema at construction (seeding the Linear-style status set), the endpoints resolve the real adapter connection, and every task mutation writes an `entity_history` row through the single adapter choke point — so the audit trail cannot disagree with the tasks table. Migration 086 is the explicit instance-layer bridge (idempotent; the adapter also self-heals on fresh installs and in tests).
- SSE liveness. The bus emits `task.*`, `work.notify`, and `ingest.work`, but the frontend listened only for `work`/`activity`/`health`/`services`/`execution`, so the board never live-updated from agent or CLI activity. `useSSE` now subscribes to all of them (plus an `onmessage` fallback) and invalidates the work queries. The CLI `update_task` and the API `update_task` action now emit `task.status_changed` carrying `updated_from` (the prior status); the ActionRegistry builds TYPED events (`task_id`/`project` first-class) so ontology listeners and GitHub sync finally fire on the API path.
- Inbox is first-class and triageable in every view (`components/tasks/InboxTriage.tsx`). An Inbox lane in Board, a collapsible Inbox section in Stream and List — the answer to "where do I triage inbox items?" is: any work view. Inline triage — promote (with project picker), snooze, dismiss — through one `useInboxTriage` hook. Ambient commitment proposals render their provenance receipt ("from iMessage · Jul 12" + source ref) parsed from the inbox line, and carry a quiet purple edge (the agent hue). The inbox `source` is now persisted so ambient items are distinguishable and dismissal (delete) is the triage decision — the proposer's `comms.db` stamp is retained, so a dismissed item is never re-proposed. `snoozed_until` hides deferred items until they elapse.
- View parity: the board card gains the shared done toggle (`useUpdateTask`, the same action every view uses); a task a session is holding shows a pulsing live dot. Absorbed from aos#68: kanban-as-a-view-toggle and inline mark-done/status/priority actions across views (its detail-panel and keyboard/DnD polish predate and remain).
- Tests: `tests/engine/work/test_board_honesty.py` (9) — history-on-mutation, old-active-not-truncated, authoritative summary, inbox source/snooze/promote, `task.status_changed` emission with `updated_from`, and the typed-event construction fix. Existing work suites unaffected. Isolated throwaway DB — never the operator's `work.db`.
