...
Summary: Kanban Phase 1 — typed states, agent delegation, and the bug task-class — plus a repair: Phase 0 (the honest board) had been silently reverted on main. The envoy commit, branched from pre-Phase-0 main, clobbered every shared file Phase 0 touched and deleted its migration + test, so v0.6.21 shipped without the honest board despite its changelog. This release re-lands Phase 0 (verified clean, envoy's own code untouched) and builds Phase 1 on top: a task can now be delegated to an agent as a real state transition, the board's columns are category-typed, and bugs arrive as a first-class task class carrying islah's 13-state pipeline without flattening its richness.

- **Re-landed Kanban Phase 0** (clobbered by the envoy merge): backend `task.status_changed` emit + `entity_history` audit writes, authoritative counts + `board_tasks`, the `statuses`/`entity_history`/`comments` tables and endpoints, typed action events, and the live-SSE frontend. Migration re-created as `087_work_board_honesty` (086 was reclaimed by rebuild-release-frontends). Restored from the last Phase-0-intact commit after verifying no envoy logic was entangled and no post-envoy change touched the restored files.
