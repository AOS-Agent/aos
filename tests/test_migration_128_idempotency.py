"""Migration 128 — work-runner decommission — idempotency and safety.

Same contract as 111–126: up() must be safe to run twice, must never touch the
...
def test_128_removes_everything_then_is_a_noop(home):
    _seed_everything(home)
    m = load_migration("128", home)
...
def test_128_preserves_other_service_declarations(home):
    _seed_everything(home)
    load_migration("128", home).up()
    assert _services(home)["disabled"] == ["n8n"]


def test_128_clears_an_explicit_opt_in(home):
    """111 deliberately left an opted-in runner alone. 128 cannot: the code it
...
def test_128_is_a_noop_on_a_machine_that_never_had_it(home):
...
def test_128_is_a_noop_with_no_config_at_all(home):
    m = load_migration("128", home)
    assert m.check() is True
    assert m.up() is True


def test_128_leaves_operator_data_alone(home):
