...
Same contract as the other v0.7.7 migrations: up() can run twice. The runner
...
named for — its neighbours are the operator's real settings, and one literal
filename is the whole retirement list. Every test runs in a sandbox HOME;
nothing reads or writes the live instance.
"""
