"""Migration 125 — close the cwd-keyed auto threads — idempotency and safety.

Same contract as 111–124: up() runs twice without the second run moving
...
def test_125_closes_only_the_unreachable_rows_then_is_a_noop(home):
...
def test_125_makes_a_backup_before_writing(home):
    db = home / ".aos" / "data" / "work.db"
    _seed(db, MIXED)
    load_migration("125", home).up()
...
def test_125_is_a_noop_when_nothing_matches(home):
    db = home / ".aos" / "data" / "work.db"
...
def test_125_is_a_noop_without_a_db(home):
    m = load_migration("125", home)
    assert m.check() is True
    assert m.up() is True


def test_125_is_a_noop_without_a_threads_table(home):
    db = home / ".aos" / "data" / "work.db"
    conn = sqlite3.connect(str(db))
...
def test_125_is_a_noop_without_a_project_id_column(home):
