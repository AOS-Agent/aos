"""
Migration 045: Sentinel autonomous commitment agent — infrastructure init.
...
def check() -> bool:
    ...
    if not CONFIG_PATH.exists():
        return False
    if not _comms_db_exists():
        return False
    try:
        conn = sqlite3.connect(str(COMMS_DB))
        ok = _table_exists(conn, "agent_triggers")
        conn.close()
        return ok
    except Exception:
        return False
...
def up() -> bool:
    ...
    # 3. Schema (comms.db may not exist yet on a brand-new install)
    if not _comms_db_exists():
        print(f"       comms.db not yet initialized at {COMMS_DB}")
        print("       Skipping schema — will be created when comms-bus first runs.")
        # Drop a marker so the comms_store consumer can pick it up later
        marker = HOME / ".aos" / "work" / "sentinel" / ".schema_pending"
        marker.touch()
        return True
