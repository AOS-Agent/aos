"""
Migration 079: Add a first-class `short_id` column to the projects table.
...
Return contract: up() returns True/None on success, False on failure, per the
strict migration-runner contract (core/infra/migrations/runner.py). Descriptive
status is printed, not returned.
"""
...
def up() -> bool:
    if not DB_PATH.exists():
        print("       Skipped: qareen.db does not exist yet")
        return True
    ...
    try:
        cols = _columns(conn, "projects")
        if not cols:
            print("       Skipped: projects table does not exist yet")
            return True
    ...
    print(f"       {'; '.join(parts) if parts else 'Already applied'}")
    return True
