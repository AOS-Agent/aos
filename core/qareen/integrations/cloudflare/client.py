...
class CloudflareClient:
    ...
    async def find_tunnel_by_name(self, name: str) -> Optional[dict]:
        """GET /accounts/{a}/cfd_tunnel?name=<name> — first live match or None."""
        ...
        for tunnel in result or []:
            if tunnel.get("deleted_at") is None:
                return tunnel
        return None
    ...
    async def ensure_otp_idp(self) -> str:
        ...
        """
        providers = await self._request(
            "GET", f"/accounts/{self._acct()}/access/identity_providers"
        )
    ...
    async def find_access_app(self, hostname: str) -> Optional[dict]:
        ...
        apps = await self._request(
            "GET", f"/accounts/{self._acct()}/access/apps"
        )
        for app in apps or []:
