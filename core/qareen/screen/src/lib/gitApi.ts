...
export interface Gate {
  ...
  source?: string;
  /** Result was stamped against an older HEAD — no longer a valid signal. */
  stale?: boolean;
}

export interface RunGatesResponse {
  started: boolean;
  ran_against?: string;
  gates?: string[];
...
// -- Ship plan (readiness + the command sequence) ---------------------------
...
export interface ShipPlanResponse {
  ready: boolean;
  blockers: string[];
  warnings: string[];
  branch: string;
...
export const gitApi = {
  ...
  runGates: (projectId: string, base = 'origin/main') =>
    api.post<RunGatesResponse>(
      `/git/${enc(projectId)}/gates/run?base=${enc(base)}`,
      {},
    ),

  shipPlan: (projectId: string, base = 'origin/main') =>
    api.get<ShipPlanResponse>(
      `/git/${enc(projectId)}/ship-plan?base=${enc(base)}`,
    ),
