...
import { useSetBatchDecision } from '@/hooks/useGit';
...
function GateChip({ gate }: { gate: Gate }) {
  ...
  return (
    <div
      ...
      title={gate.summary || gate.id}
    >
      {v.icon}
      <span className="font-mono">{gate.id}</span>
      {gate.summary && <span className="text-text-quaternary truncate max-w-[160px]">· {gate.summary}</span>}
    </div>
  );
}
...
export default function ShipLedger({
  ...
  return (
    <div>
      ...
      {/* Plan-level gates (ship-map blockers) */}
      {gates.length > 0 && (
