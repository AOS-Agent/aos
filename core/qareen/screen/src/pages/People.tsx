        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <button onClick={() => { setShowCompose(!showCompose); if (!composeChannel && channels.length > 0) setComposeChannel(channels[0][0]); }}
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-[5px] border text-[11px] font-[510] transition-colors cursor-pointer ${showCompose ? 'bg-accent text-on-accent border-accent' : 'bg-bg-secondary border-border-secondary text-text-tertiary hover:text-text-secondary hover:bg-bg-tertiary'}`}
              style={{ transitionDuration: 'var(--duration-instant)' }}
        ...
        <div className="shrink-0 px-6 py-3 border-b border-border bg-bg-secondary/50">
          <div className="flex items-center gap-2 mb-2">
            {channels.map(([ch]) => (
              <button key={ch} onClick={() => setComposeChannel(ch)}
                className={`px-2 py-0.5 rounded-[3px] text-[10px] font-[510] capitalize transition-colors cursor-pointer ${composeChannel === ch ? 'bg-accent text-on-accent' : 'bg-bg-tertiary text-text-tertiary hover:text-text-secondary'}`}
                style={{ transitionDuration: 'var(--duration-instant)' }}
              >{ch}</button>
            ))}
          </div>
          <div className="flex items-center gap-2">
            ...
            <button onClick={handleSend} disabled={!composeText.trim() || sendMutation.isPending}
              className="flex items-center justify-center w-8 h-8 rounded-[5px] bg-accent text-on-accent hover:bg-accent-hover transition-colors cursor-pointer disabled:opacity-50"
              style={{ transitionDuration: 'var(--duration-instant)' }}
