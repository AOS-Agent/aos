...
export function useRemoteAccessStatus() {
  return useQuery({
    ...
    queryFn: async (): Promise<RAStatus> => {
      ...
      // Backend is flat per the RAStatus contract ({ status, hostname, ...,
      // connector_health }). Keep a defensive guard for a legacy
      // { state, connector_health } shape: unwrap json.state if present, and
      // always surface connector_health at the top level so consumers never read
      // undefined.
      const json = await res.json();
      return { ...(json.state ?? json), connector_health: json.connector_health ?? null };
    },
    staleTime: 5_000,
...
export function useRemoteAccessProgress(): {
  steps: RAProgress[];
  ready: boolean;
  reset: () => void;
} {
  const [steps, setSteps] = useState<RAProgress[]>([]);
  // `ready` flips true once the EventSource is actually open (es.onopen). The
  // wizard gates connect.mutate on ready===true so the POST /connect cannot race
  // ahead of this subscription and drop the early 'start'/'tunnel' steps.
