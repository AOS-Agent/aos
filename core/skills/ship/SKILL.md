...
description: >
  ...
  main and onto the local runtime.
...
> "Ship this to main? The runtime updates right after."
...
### Step 4: Apply to the Runtime

This is where the ship ends. Push the update onto this machine so what is
running matches what was just shipped. `~/aos` is a release symlink (no
`.git`) on modern installs — never `git pull` it; go through the update
pipeline:
...
service restart), that is a failed ship — fix it now, not later. Never
record a ship as done on the strength of the push alone; the push is the
easy half. The failure is also recorded in
`~/.aos/data/update-state.json` under `last_apply`.

Then verify, rather than assuming the absence of an ERROR line means
success:
...
- ALWAYS apply to ~/aos/ after pushing so the operator runs what they
  shipped — a ship that only reached main is not a ship
- ALWAYS verify the applied runtime (`aos self-test`, `aos hygiene`)
  rather than trusting that no ERROR line means no error
